...
private const val camel = """Switching on the camera in the camel habitat...
 ___.-''''-.
/___  @    |
',,,,.     |         _.'''''''._
     '     |        /           \
...
private const val lion = """Switching on the camera in the lion habitat...
                                               ,w.
                                             ,YWMMw  ,M  ,
                        _.---.._   __..---._.'MMMMMw,wMWmW,
                   _.-""        '''           YP"WMMMMMMMMMb,
...
private const val deer = """Switching on the camera in the deer habitat...
   /|       |\
`__\\       //__'
   ||      ||
 \__`\     |'__/
...
private const val goose = """Switching on the camera in the goose habitat...
...
private const val bat = """Switching on the camera in the bat habitat...
...
private const val rabbit = """Switching on the camera in the rabbit habitat...
         ,
        /|      __
       / |   ,-~ /
      Y :|  //  /
...
fun main() {
    do {
        ...
        println("Please enter the number of the habitat you would like to view:$userInput")
        if (userInput == "exit") println("See you later!")
        if (userInput == "0") println(camel)
        if (userInput == "1") println(lion)
        if (userInput == "2") println(deer)
        if (userInput == "3") println(goose)
        if (userInput == "4") println(bat)
        if (userInput == "5") println(rabbit)
